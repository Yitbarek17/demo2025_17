...
const ProjectForm: React.FC<ProjectFormProps> = ({ project, metadata, onSubmit, onCancel }) => {
  ...
  const validateForm = () => {
    ...
    else if (formData.subSector === 'Other' && !otherSubSector.trim()) {
      newErrors.subSector = 'Please specify the sub-sector';
    }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <InputField 
              ...
              isDark={isDark}
              showOtherInput={true}
              otherValue={otherSubSector}
              onOtherChange={handleOtherSubSectorChange}
            />
            <InputField 
              label={t('form.region')} 
              name="region" 
